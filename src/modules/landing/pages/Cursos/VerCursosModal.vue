<template>
  <div class="modal-overlay" v-if="showModal && curso">
    <div class="modal">
      <h2>Detalles del Curso</h2>
      <div class="details">
        <div><strong>ID:</strong> {{ curso.id }}</div>
        <div><strong>Curso:</strong> {{ curso.nombre_curso }}</div>
        <div><strong>Descripcion:</strong> {{ curso.descripcion }}</div>
        <div><strong>F. Creación:</strong> {{ curso.created_at }}</div>
      </div>
...
export default defineComponent({
  ...
  props: {
    ...
    curso: {
      type: Object as PropType<Curso | null>,
      required: true
    }
  },
  setup(props, { emit }) {
    ...
    watch(() => props.curso, async (newVal) => {
      if (newVal) {
        try {
          ...
          const response = await axios.get(`/curso/${newVal.id}/estudiantes`);
