...
<template>
  ...
  export default defineComponent({
    ...
    props: {
      ...
      profesor: {
        type: Object as PropType<Profesor | null>,
        required: true
      }
    },
    emits: ['close','update'],
    setup(props, { emit }) {
      const form = ref({ ...props.profesor });
  
      watch(() => props.profesor, (newVal) => {
        if (newVal) {
          form.value = { ...newVal };
        }
      });
      ...
      const handleSubmit = async () => {
        try {
          await axios.put(`/profesor/${form.value.id}`, form.value);
